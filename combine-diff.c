			       ...
			       long flags)
{
	if (flags & XDF_WHITESPACE_FLAGS) {
		for (; len1 > 0 && XDL_ISSPACE(line1[len1 - 1]); len1--) continue;
		for (; len2 > 0 && XDL_ISSPACE(line2[len2 - 1]); len2--) continue;
	}
	...
	while (len1 > 0 && len2 > 0) {
		...
		if (XDL_ISSPACE(line1[len1]) || XDL_ISSPACE(line2[len2])) {
			...
			for (; len1 > 0 && XDL_ISSPACE(line1[len1]); len1--) continue;
			for (; len2 > 0 && XDL_ISSPACE(line2[len2]); len2--) continue;
		}
